...
export function ProductModifierModal({
  ...
  if (!item) return null;
  // Use modifiers from item, or fallback to empty array
  const modifierSections = item.modifiers || [];
  ...
  const calculateTotal = () => {
    ...
    // Add size modifier
    const sizeOption = modifierSections.find((s: any) => s.type === 'single')?.options?.find((o: any) => o.name === selectedSize);
    if (sizeOption) total += Number(sizeOption.price);
    // Add selected add-ons
    modifierSections.filter((s: any) => s.type === 'multiple').forEach((section: any) => {
      (section.options || []).forEach((option: any) => {
        if (selectedAddons.has(option.name)) {
  ...
  const handleSubmit = () => {
    ...
    // Reset state
    setQuantity(1);
    setSelectedSize(''); // Reset to empty or first option if we want
            ...
            {modifierSections.map((section, sectionIndex) => <div key={sectionIndex} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                {(section.options || []).map((option: any, index: number) => {
                  // Use name as ID for now since we don't have explicit IDs in the new dynamic structure
                  const optionId = option.name;
                  const isSelected = section.type === 'single' ? selectedSize === optionId : selectedAddons.has(optionId);
                  return <motion.button key={index} whileTap={{
                    scale: 0.97
                  }} onClick={() => {
                    if (section.type === 'single') {
                      // For single select, we store the Option Name as the ID/value for simplicity
                      setSelectedSize(option.name);
                    } else {
                      handleToggleAddon(option.name);
                    }
                  }} className={`
                    ...
                    {Number(option.price) !== 0 && <span className={`text-xs font-bold ${isSelected ? 'text-white' : 'text-white/60'}`}>
