        ...
        stiffness: 200
            ...
            <div className="w-full max-w-md flex flex-col items-center">
              ...
              <div className="grid grid-cols-3 gap-4 w-full max-w-[300px]">
                {['1', '2', '3', '4', '5', '6', '7', '8', '9', '', '0'].map((key, i) => key === '' ? <div key={i} /> : <button key={key} onClick={() => handleKeyPress(key)} className="h-16 w-full rounded-full bg-white/5 border border-white/10 text-2xl font-bold text-white active:bg-[#FF6B00] active:border-[#FF6B00] transition-colors">
                        {key}
                      </button>)}
                <button onClick={handleDelete} className="h-16 w-full rounded-full bg-white/5 border border-white/10 flex items-center justify-center text-white/60 active:bg-red-500/20 active:text-red-500 active:border-red-500/50 transition-colors">
                  <Delete size={24} />
                </button>
              </div>
            </div>
